...
- Канальный уровень - Обеспечивает надежную передачу кадров данных между узлами, подключенными к одной физической среде (Ethernet, Wi-Fi) <!-- .element: class="fragment" -->
- Сетевой уровень - Отвечает за определение пути передачи данных и маршрутизацию пакетов между узлами в сети (IP) <!-- .element: class="fragment" -->

------

- Транспортный уровень - Обеспечивает надежную передачу данных между узлами (TCP, UDP)
- Сеансовый уровень - Управляет сессиями и диалогами между приложениями (PPTP, RPC) <!-- .element: class="fragment" -->
- Представительный уровень - Отвечает за преобразование данных в формат, пригодный для передачи и чтения на различных устройствах (SSL/TLS, JPEG, MPEG) <!-- .element: class="fragment" -->
- Прикладной уровень - Предоставляет интерфейсы и протоколы для взаимодействия с приложениями (HTTP, FTP, SMTP) <!-- .element: class="fragment" -->
...
- Интернет уровень - Отвечает за маршрутизацию и передачу пакетов данных между узлами (IP) <!-- .element: class="fragment" -->
- Транспортный уровень - Обеспечивает надежную передачу данных между приложениями (TCP, UDP) <!-- .element: class="fragment" -->
- Прикладной уровень - Предоставляет сетевые услуги и протоколы для приложений (HTTP, FTP, SMTP, DNS) <!-- .element: class="fragment" -->
